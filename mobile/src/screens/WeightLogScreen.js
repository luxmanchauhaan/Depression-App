...
import { submitWeightLog, getWeightHistory } from '../api';
import { colors, spacing, radius, shadow } from '../theme';

const THEME = { bg: '#DFF5E3', icon: '#5FAE7B' };

export default function WeightLogScreen({ token, onNavigate }) {
  ...
  async function loadHistory() {
    setLoading(true);
    try {
      const result = await getWeightHistory(token);
      setHistory(result.history);
    } catch (err) {
      console.log('Failed to load weight history:', err.message);
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleSubmit() {
    ...
    if (!weight || isNaN(weightNum) || weightNum <= 0 || weightNum > 500) {
      Alert.alert('Invalid input', 'Enter a valid weight in kg.');
      return;
    }

    setSubmitting(true);
    try {
      await submitWeightLog(token, weightNum);
      setWeight('');
      await loadHistory();
      Alert.alert('Logged', 'Your weight entry has been saved.');
    } catch (err) {
  ...
  const chartData = {
    ...
    datasets: [{ data: chronological.map((item) => parseFloat(item.weight_kg)) }],
  };
    ...
    <View style={styles.container}>
      <View style={[styles.header, { backgroundColor: THEME.icon }]}>
        <View style={[styles.headerIconWrap, { backgroundColor: THEME.bg }]}>
          <Ionicons name="scale-outline" size={28} color={THEME.icon} />
        </View>
        <Text style={styles.headerTitle}>Weight Log</Text>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.body}>
        <View style={styles.card}>
          ...
          <TouchableOpacity
            ...
            disabled={submitting}
          >
            <Text style={styles.primaryButtonText}>{submitting ? 'Saving...' : 'Log Weight'}</Text>
          </TouchableOpacity>
        </View>
          ...
          <>
            ...
            {history.length >= 2 && (
              <View style={styles.chartCard}>
                <Text style={styles.chartCaption}>Weight over time (kg)</Text>
                <View>
                  <LineChart
                    data={chartData}
                    width={screenWidth}
                    ...
                    onDataPointClick={({ index, x, y }) => {
                      ...
                      setTooltip({ x, y, date: new Date(item.logged_date).toLocaleDateString(), value: `${item.weight_kg} kg` });
                    }}
                  />
                  {tooltip && (
                ...
                <Text style={styles.tapHint}>Tap a point to see date and value</Text>
              </View>
            )}
